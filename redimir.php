...
#region [Funciones Puntos Leal]
    if ($voucher_provider == 'PUNTOS_LEAL') { // si el premio es un bono de Puntos leal
        ...
        if ($swSave) {
            ...
            header('location:/progreso-redenciones');
            exit;
        } else {
            header("Location: /exit");
            die();
    ...
    if ($idbono != "" and $brand != "") {
        ...
        $responseOutputText = json_encode($output["response"]);
        if ($idtxt != "" and $json != "") {
           ...
           // $_SESSION['totalRedemptions'] += 1;
            ...
            $emailsArray = ['alexander.cardona@isobar.com', 'carlos.torres@isobar.com','jeferson.valeta@isobar.com'];
